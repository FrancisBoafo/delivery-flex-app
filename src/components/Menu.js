          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 xl:gap-x-8">
            {restaurants.map((restaurant) => (
              <a key={restaurant.id} href={restaurant.href} className="group">
                ...
                <h3 className="mt-4 text-sm text-left text-gray-700">{restaurant.name}</h3>
                <p className="mt-1 text-lg text-left font-medium text-gray-800">{restaurant.location}</p>
              </a>
            ))}
          </div>
        </div>
